...
// src/index.ts
var app = new Hono2();
var APP_ORIGIN = "https://family-budget-app-2v5.pages.dev";
app.use("*", async (c, next) => {
...
app.post("/api/auth/login", async (c) => {
  ...
  const expiresAt = new Date(Date.now() + 1e3 * 60 * 60 * 24 * 14).toISOString();
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
app.post("/api/account/reconcile", requireUser, async (c) => {
  ...
  await c.env.DB.prepare(
    `UPDATE account_state
     ...
     WHERE id = 'main'`
  ).bind(bankBalance, (/* @__PURE__ */ new Date()).toISOString()).run();
  return c.json({ ok: true });
});
...
app.get("/api/spend/summary", requireUser, async (c) => {
  ...
  const byCategory = CATEGORIES.filter((cat) => cat.id !== "income").map((cat) => {
  ...
  const eventsRows = await c.env.DB.prepare(
    `SELECT id, title, start_at, end_at, location
     ...
     WHERE user_id = ?
       ...
       AND start_at < ?
     ORDER BY start_at ASC`
  ).bind(userId, now.toISOString(), calEnd.toISOString()).all();
  return c.json({
...
// .wrangler/tmp/bundle-RcNfvZ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RcNfvZ/middleware-loader.entry.ts
