...
// ─── Types ─────────────────────────────────────────────────────────────────────
type ScreenId = "welcome" | "features" | "household" | "householdSize" | "budgetStyle" | "done";
...
function nextScreen(cur: ScreenId, features: string[], householdType: string): ScreenId {
  ...
  if (cur === "householdSize") return features.includes("budgeting") ? "budgetStyle" : "done";
  if (cur === "budgetStyle")   return "done";
  return cur;
}
  ...
  async function finish() {
    ...
    try {
      await api("/api/onboarding/complete", {
        method: "POST",
        body: JSON.stringify({ quizAnswers, startingBalance: null }),
      });
